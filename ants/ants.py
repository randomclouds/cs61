...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            flag = self.can_contain(place.ant) or place.ant.can_contain(self)
            assert flag is True, 'Too many ants in {0}'.format(place)
            if place.ant.is_container is False:
...
# The WallAnt class
class WallAnt(Ant):
    '''The ant just do nothin each turn'''
    name = 'Wall'
    food_cost=4
    implemented=True
...
# The HungryAnt Class
class HungryAnt(Ant):
    """The ant will eat the bee in its place"""
    name = 'Hungry'
    implemented = True
    food_cost=4
    chew_cooldown=3
    ...
    def action(self, gamestate):
        if self.cooldown:
            self.cooldown+=1
        if self.cooldown > self.chew_cooldown+1:
            self.cooldown = 0
...
class TankAnt(ContainerAnt):
    """The TankAnt will defend the ant which is in the same place"""
    name = 'Tank'
    implemented = True
    food_cost = 6
    damage = 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        p = self.place
        while p and p.is_hive is False:
            if p.bees:
                for bee in p.bees:
                    distance_dic[bee] = distance
            if p.ant:
                if p.ant.is_container:
                    distance_dic[p.ant] = distance
                    if p.ant.ant_contained and p != self.place:
                        distance_dic[p.ant.ant_contained] = distance
                elif p!= self.place:
                    distance_dic[p.ant] = distance
            p = p.entrance
            distance += 1
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not hasattr(self, 'scary_len'):
            self.scary_len = length
        # END Problem EC 2
